...
from ultralytics_yolo11.utils import LOGGER, DataExportMixin, SimpleClass, TryExcept, checks, plt_settings
...
def bbox_iou(
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU:
        ...
        if SIoU:  # SCYLLA-IoU https://arxiv.org/abs/2205.12740 (angle + distance + shape cost)
            s_cw = (b2_x1 + b2_x2 - b1_x1 - b1_x2) * 0.5  # center-x offset (box2 - box1)
            s_ch = (b2_y1 + b2_y2 - b1_y1 - b1_y2) * 0.5  # center-y offset
            sigma = (s_cw.pow(2) + s_ch.pow(2)).sqrt() + eps  # center distance
            sin_alpha = torch.where(
                (s_cw.abs() / sigma) > (2**0.5 / 2), s_ch.abs() / sigma, s_cw.abs() / sigma
            )
        ...
        if CIoU or DIoU or EIoU:  # Distance / Complete / Efficient IoU https://arxiv.org/abs/1911.08287v1
            ...
            if EIoU:  # Efficient IoU https://arxiv.org/abs/2101.08158 (penalize w/h directly)
                rho_w2 = (w1 - w2).pow(2)
                rho_h2 = (h1 - h2).pow(2)
                return iou - (rho2 / c2 + rho_w2 / (cw.pow(2) + eps) + rho_h2 / (ch.pow(2) + eps))  # EIoU
...
def bbox_nwd(box1: torch.Tensor, box2: torch.Tensor, xywh: bool = False, constant: float = 12.7, eps: float = 1e-7):
    """Normalized Wasserstein Distance (NWD) similarity between bounding boxes.

    Models each box as a 2D Gaussian and returns exp(-W2 / constant) in (0, 1]. Far less sensitive to
    small positional/scale errors than IoU, so it is well suited to tiny objects (e.g. canker spots).
    Reference: https://arxiv.org/abs/2110.13389. `constant` is dataset-dependent (avg absolute size).
    """
    if xywh:
        ...
        b1_x1, b1_y1, b1_x2, b1_y2 = box1.chunk(4, -1)
        b2_x1, b2_y1, b2_x2, b2_y2 = box2.chunk(4, -1)
        cx1, cy1, w1, h1 = (b1_x1 + b1_x2) / 2, (b1_y1 + b1_y2) / 2, b1_x2 - b1_x1, b1_y2 - b1_y1
        cx2, cy2, w2, h2 = (b2_x1 + b2_x2) / 2, (b2_y1 + b2_y2) / 2, b2_x2 - b2_x1, b2_y2 - b2_y1
    w2_dist = (cx1 - cx2).pow(2) + (cy1 - cy2).pow(2) + ((w1 - w2) / 2).pow(2) + ((h1 - h2) / 2).pow(2)
    return torch.exp(-(w2_dist + eps).sqrt() / constant)
