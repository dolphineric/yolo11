...
from ultralytics_yolo11.utils.torch_utils import fuse_conv_and_bn

from .conv import Conv, CoordinateAttention, DWConv, GhostConv, LightConv, RepConv, autopad
...
class C3k2_CA(C3k2):
    """C3k2 module enhanced with Coordinate Attention.

    This module extends C3k2 by appending a CoordinateAttention block after the output convolution,
    enabling the network to capture long-range spatial dependencies with precise positional information.

    Attributes:
        ca (CoordinateAttention): Coordinate attention module applied after C3k2 output.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through C3k2_CA module.
...
class DCNv2(nn.Module):
    """Modulated Deformable Convolution v2 + BatchNorm + activation (drop-in for a Conv).

    The sampling grid offsets and modulation mask are predicted from the input, letting the
    receptive field adapt to irregular object shapes (e.g. citrus leafminer trails).
    """

    default_act = nn.SiLU()  # default activation

    def __init__(self, c1: int, c2: int, k: int = 3, s: int = 1, p=None, g: int = 1, d: int = 1, act=True):
        """Initialize modulated deformable conv with same arg layout as `Conv`."""
        super().__init__()
        p = autopad(k, p, d)
        self.k = k
        # per deformable group: 2*k*k offset channels + k*k modulation-mask channels
        self.offset_mask = nn.Conv2d(c1, g * 3 * k * k, k, s, p, dilation=d)
        self.dcn = DeformConv2d(c1, c2, k, stride=s, padding=p, dilation=d, groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
        # start as a plain conv (zero offset/mask) for stable training
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if torch.are_deterministic_algorithms_enabled() and not torch.is_deterministic_algorithms_warn_only_enabled():
            # deform_conv2d backward has no deterministic kernel -> relax to warn-only
            torch.use_deterministic_algorithms(True, warn_only=True)
        # torchvision deform_conv2d has no autocast kernel -> run the whole block in fp32, then cast
        # back to the input dtype so the module behaves like a normal AMP-safe conv.
        with torch.autocast(device_type=x.device.type, enabled=False):
            xf = x.to(self.dcn.weight.dtype)  # match param dtype (fp32 in AMP train, fp16 in half val)
            if min(x.shape[-2:]) < self.k:
                # spatial too small to deform (e.g. 1x1 during FLOPs profiling) -> plain conv fallback
                out = F.conv2d(
                    xf, self.dcn.weight, self.dcn.bias, self.dcn.stride, self.dcn.padding,
                    self.dcn.dilation, self.dcn.groups,
                )
            else:
                o1, o2, mask = self.offset_mask(xf).chunk(3, dim=1)
                # nan_to_num guards against NaN/Inf offsets (e.g. from the uninitialized torch.empty
                # input used during FLOPs profiling), which would crash deform_conv2d's sampler.
                offset = torch.nan_to_num(torch.cat((o1, o2), dim=1))
                out = self.dcn(xf, offset, mask.sigmoid())
            out = self.act(self.bn(out))
        return out.to(x.dtype)


class Bottleneck_DCN(nn.Module):
    """Standard bottleneck whose second 3x3 convolution is deformable (DCNv2)."""

    def __init__(self, c1: int, c2: int, shortcut: bool = True, g: int = 1, k=(3, 3), e: float = 0.5):
        """Initialize a deformable bottleneck (same signature as `Bottleneck`)."""
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, k[0], 1)
        self.cv2 = DCNv2(c_, c2, k[1], 1, g=g)
        self.add = shortcut and c1 == c2

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply deformable bottleneck with optional shortcut connection."""
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))


class C3k2_DCN(C3k2):
    """C3k2 variant whose inner bottlenecks use modulated deformable convolution.

    Interface-compatible with `C3k2`, so it is parsed identically and can replace `C3k2`
    in any YAML. Intended for irregular lesion shapes (CLM).
    """
    ...
    def __init__(
        ...
        """Initialize C3k2_DCN; `c3k`/`attn` kept for signature compatibility (uses DCN bottlenecks)."""
        super().__init__(c1, c2, n, c3k, e, attn, g, shortcut)
        self.m = nn.ModuleList(Bottleneck_DCN(self.c, self.c, shortcut, g) for _ in range(n))
