...
from ultralytics_yolo11.utils.metrics import OKS_SIGMA, RLE_WEIGHT
from ultralytics_yolo11.utils.ops import crop_mask, xywh2xyxy, xyxy2xywh
from ultralytics_yolo11.utils.tal import RotatedTaskAlignedAssigner, TaskAlignedAssigner, dist2bbox, dist2rbox, make_anchors
from ultralytics_yolo11.utils.torch_utils import autocast
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, iou_type: str = "ciou"):
        """Initialize the BboxLoss module with regularization maximum, DFL and box-IoU type settings."""
        super().__init__()
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None
        self.iou_type = (iou_type or "ciou").lower()  # ciou|diou|giou|siou|eiou|wiou|wiouv3|nwd
        self.register_buffer("iou_mean", torch.ones(1), persistent=False)  # WIoU v3 running mean of (1-IoU)
    ...
    def forward(
        ...
        loss_iou = self._iou_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask], weight, target_scores_sum)
    ...
    def _iou_loss(self, pred: torch.Tensor, target: torch.Tensor, weight: torch.Tensor, target_scores_sum):
        """Compute the weighted box-regression loss using the configured IoU type (xyxy boxes)."""
        t = self.iou_type
        if t in {"wiou", "wiouv1", "wiouv3"}:  # Wise-IoU https://arxiv.org/abs/2301.10051
            iou = bbox_iou(pred, target, xywh=False)  # plain IoU, shape (N, 1)
            l_iou = 1.0 - iou
            ...
            c2 = cw.pow(2) + ch.pow(2) + 1e-7  # convex diagonal squared
            rho2 = ((b2x1 + b2x2 - b1x1 - b1x2).pow(2) + (b2y1 + b2y2 - b1y1 - b1y2).pow(2)) / 4
            r_wiou = torch.exp(rho2 / c2.detach())  # distance amplifier (detached denominator)
            if t == "wiouv3":  # non-monotonic focusing on ordinary-quality anchors
                with torch.no_grad():
                    self.iou_mean.mul_(0.99).add_(0.01 * l_iou.mean())
                beta = l_iou.detach() / (self.iou_mean + 1e-7)
                alpha, delta = 1.9, 3.0
                per = (beta / (delta * alpha ** (beta - delta))) * r_wiou * l_iou
            else:
                per = r_wiou * l_iou
            return (per * weight).sum() / target_scores_sum
        if t == "siou":
            iou = bbox_iou(pred, target, xywh=False, SIoU=True)
        elif t == "eiou":
            iou = bbox_iou(pred, target, xywh=False, EIoU=True)
        elif t == "giou":
            iou = bbox_iou(pred, target, xywh=False, GIoU=True)
        elif t == "diou":
            iou = bbox_iou(pred, target, xywh=False, DIoU=True)
        elif t == "nwd":  # Normalized Wasserstein Distance (small-object friendly)
            iou = bbox_nwd(pred, target, xywh=False)
        else:  # ciou (default, original behaviour)
